...
try:
    ...
    raise ImportError(
        'Please install `rich`, `prompt-toolkit` and `argcomplete` to use the Strategic Advisor CLI, '
        'you can run `pip install "pydantic-ai-slim[cli]"`'
    ) from _import_error


try:
    __version__ = version('strategic-advisor')
except:
    __version__ = "0.1.0"
...
def cli(args_list: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog='advisor',
        description=f"""\
Strategic Advisor CLI v{__version__}\n\n
...
""",
    ...
    console = Console()
    console.print(
        f'[green]Strategic Advisor v{__version__} using[/green] [magenta]{args.model}[/magenta]', highlight=False
    )
    if args.version:
        return 0
    if args.list_models:
    ...
    try:
        advisor_agent.model = infer_model(args.model)
    ...
    if prompt := cast(str, args.prompt):
        try:
            asyncio.run(ask_agent(advisor_agent, prompt, stream, console, code_theme))
        except KeyboardInterrupt:
            pass
        return 0

    history = Path.home() / '.advisor-prompt-history.txt'
    ...
    try:
        return asyncio.run(run_chat(session, stream, advisor_agent, console, code_theme))
...
async def run_chat(session: PromptSession[Any], stream: bool, agent: Agent, console: Console, code_theme: str) -> int:
    ...
    while True:
        try:
            ...
            text = await session.prompt_async('advisor � ', auto_suggest=auto_suggest, multiline=multiline)
